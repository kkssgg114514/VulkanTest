...
//图像库
//将stb的函数实现包含进来
//由于在.h中声明会导致main.obj重复定义的错误,所以只能在.cpp中声明
...
void TriangleApplication::initWindow()
{
	...
	//由于GLFW一开始是为了OpenGL创建的,因此需要显式阻止其创建OpenGL的上下文
...
void TriangleApplication::cleanup()
{
	...
	//清除窗口表面,需要实例,所以要在实例清除之前完成
	vkDestroySurfaceKHR(instance, surface, nullptr);

	//清除Vulkan实例
	vkDestroyInstance(instance, nullptr);

	//一旦窗口关闭,就可以结束GLFW,清除资源
...
void TriangleApplication::createLogicalDevice()
{
	...
	//使用多个队列族时创建逻辑设备的过程,set可以避免相同的索引,也就可以避免
	std::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
	std::set<int> uniqueQueueFamilies = { indices.graphicsFamily, indices.presentFamily };

	//队列优先级权重位于0.0f到1.0f之间,即使只有一个队列,也要显式的赋予优先级
	...
	for (int queueFamily : uniqueQueueFamilies)
	{
		...
		//目前只使用了一个,大部分时候都只用一个(一个队列族中用一个)
		queueCreateInfo.queueCount = 1;
		queueCreateInfo.pQueuePriorities = &queuePriority;
		queueCreateInfos.push_back(queueCreateInfo);
	}

	//指定应用程序使用的设备特性,各向异性采样需要手动开
	VkPhysicalDeviceFeatures deviceFeatures {};
...
bool TriangleApplication::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
	...
	//将所需扩展保存在一个集合中,利用可用扩展的相同元素消除所需扩展集合中的元素,消除完全说明满足所需
...
void TriangleApplication::createSwapChain()
{
	...
	{
		//共享图像所有权需要至少两个队列族,当图形队列和呈现队列不是一个队列族时使用协同模式
...
void TriangleApplication::createGraphicsPipeline()
{
	...
	//输入装配,顶点数据定义了那种类型的图元,是否启动几何图元重启
...
void TriangleApplication::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples,
...
void TriangleApplication::loadModel()
{
	...
	for (const auto& shape : shapes)
	{
		for (const auto& index : shape.mesh.indices)
		{
			...
			//OBJ默认以左下角为原点,而Vulkan默认为左上角,将y轴坐标翻转即可[0,1]区间内
			vertex.texCoord = {
...
VkSampleCountFlagBits TriangleApplication::getMaxUsableSampleCount()
{
	...
	VkSampleCountFlags counts = std::min(physicalDeviceProperies.limits.framebufferColorSampleCounts,
										 physicalDeviceProperies.limits.framebufferDepthSampleCounts);
